...
import { babyFilter } from "../../../utils/filterImages"
...
const ChildCategoryFilter = () => {
  ...
  // Find the decorations category
  const decorationsCategory = useMemo(() => {
    return categories.find(cat => cat.category_name === "Decorations");
  }, [categories]);

  // Get child categories as an array
  const childCategories = useMemo(() => {
    if (!decorationsCategory || !decorationsCategory.child_category) return [];
    return Object.entries(decorationsCategory.child_category).map(([id, data]) => ({
      id,
      name: data.name,
  ...
  const handleCategoryClick = (childSlug, childId) => {
    if (activeCategory === childId) {
      ...
      navigate(`/decorations/${childSlug}`);
    }
  };

  if (!decorationsCategory) return null;

  console.log("Bhai Dekh le kya kya aaya hai ", childCategories)
